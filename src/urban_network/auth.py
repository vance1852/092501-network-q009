...
class Auth:
    def __init__(self,db,lock=None):
        self.db=db; self.lock=lock or threading.RLock()
        with self.lock:
            self.db.execute("CREATE TABLE IF NOT EXISTS users(user_id TEXT PRIMARY KEY,role TEXT NOT NULL,salt TEXT NOT NULL,password_hash TEXT NOT NULL,active INTEGER NOT NULL DEFAULT 1,created_at TEXT NOT NULL)"); self.db.execute("CREATE TABLE IF NOT EXISTS sessions(token TEXT PRIMARY KEY,user_id TEXT NOT NULL,expires_at TEXT NOT NULL,active INTEGER NOT NULL DEFAULT 1)"); self.db.commit()
    def create_user(self,user_id,password,role="operator"):
        ...
        with self.lock: self.db.execute("INSERT INTO users VALUES(?,?,?,?,1,?)",(user_id,role,salt,_digest(password,salt),utcnow())); self.db.commit()
    ...
    def deactivate(self,user_id):
        with self.lock: self.db.execute("UPDATE users SET active=0 WHERE user_id=?",(user_id,)); self.db.execute("UPDATE sessions SET active=0 WHERE user_id=?",(user_id,)); self.db.commit()
